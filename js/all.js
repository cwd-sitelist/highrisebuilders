...
;(function($, window, document, underfined) {
    ...
    $(window).on('load', function () {
        $('body').addClass('loaded').find('.highrise-loader').addClass('is-loaded');
        startBtnAnimate();
        fixShowcase();
        startFigureAnimate();
    ...
    /**********************************/
    // social icons
    $('.highrise-social-nav > a').on('click', function (e) {
        var thisItem = $(this);
        var thisParent = $(this).parent('.highrise-social-nav');
    ...
    function openSimpleMenu() {
        $('.highrise-nav-menu-icon').toggleClass('active');
        $('.highrise-navigation').slideToggle();
        $('.highrise-header').toggleClass('open-menu');
    ...
    $('.highrise-nav-menu-icon a').on('click', function (e) {
        // openSimpleMenu();
        // e.preventDefault();
    });
    ...
    // menu clone link
    $('.highrise-header.full .menu-item-has-children').each(function () {
        var $this = $(this),
    ...
    function moveMenu(direction) {
        ...
        $('.highrise-header.full nav').css('transform', 'translateX(-' + (show_level * 100) + '%)');

        if (show_level > 0) {
            $('.highrise-header-form').addClass('hidyy');
        } else {
            $('.highrise-header-form').removeClass('hidyy');
        }

    }

    $('.highrise-header.full .main-menu a').on('click', function (e) {
    ...
    if ($('.highrise-navigation').length) {
        // swipe menu events
        var hammertime = new Hammer($('.highrise-navigation-inner')[0], {});
    ...
    function topBannerHeight() {
        var headerH = $('.highrise-header').outerHeight() || 0;
        var footerH = $('.highrise-footer').not('.highrise-footer.modern').outerHeight() || 0;
    ...
    function setHeightAllIframe() {
        // $('.highrise-iframe-wrapper').each(function(){
    ...
    function comingSoonValue() {
        if ($('.highrise-coming-time-wrapper .coming-time-item').length) {

            var comingSoonElements = $('.highrise-coming-time-wrapper .coming-time-item');
    ...
    function updateClock($clock, endTime, updateDays) {
        ...
        if (t.total <= 0) {
            clearInterval(highrise_timeinterval);
        }
    }

    if ($('.highrise-coming-time-wrapper').length) {
        var comingTimeWrapper = $('.highrise-coming-time-wrapper');
        ...
        comingTimeWrapper.each(function () {
            ...
            highrise_timeinterval = setInterval(function () {
    ...
    function initVivus() {
        ...
        $('.highrise-svg').each(function (index) {
    ...
    function filmstripSliderHeight() {
        if ($('.highrise_filmstrip').not('no-footer-content')) {
            var filmstripWrapperHeight = $('.highrise_filmstrip').height(),
    ...
    /**********************************/
    // DROPDOWN MENU
    var dropdown = document.querySelectorAll('.highrise-dropdown');
    var dropdownArray = Array.prototype.slice.call(dropdown, 0);
    dropdownArray.forEach(function (el) {
        var button = el.querySelector('a[data-toggle="dropdown"]'),
            menu = el.querySelector('.highrise-dropdown-menu'),
    ...
    // FILTER
    $('.filmstrip-filter .highrise-dropdown-menu li').on('click', function (e) {
        var filterValue = '.' + $(this).attr('data-filter');
        $('.filmstrip-slider').slick('slickUnfilter');
        $('.filmstrip-slider').slick('slickFilter', filterValue);

        $('.highrise-dropdown a[data-toggle="dropdown"]').removeClass('active');
        $('.highrise-dropdown-menu').removeClass('showee');
        $('.highrise-dropdown-menu').addClass('hidee');
        $('.highrise-dropdown i.icon-arrow').removeClass('openee');
        $('.highrise-dropdown i.icon-arrow').addClass('closee');
    ...
    function openTopFilter() {
        ...
        $('.highrise_filter_projects').slideToggle();
        $('.highrise-header').toggleClass('open-filter');
        $('body, html').toggleClass('no-scroll');
    }
    ...
    $('.highrise_filter_projects .highrise_filter_item li').on('click', function (e) {
        $(this).toggleClass('active');
        e.preventDefault();
    });

    $('.highrise_filter_link_wrapper .filter-btn').on('click', function (e) {

        var filterValue = [];
        $('.highrise_filter_projects [data-filter].active').each(function (i) {
        ...
        $('.highrise_services, .highrise_books, .highrise_exhibition_grid, .highrise_grid, .highrise_masonry').isotope({
    ...
    $('.filter-clear-all').on('click', function (e) {
        $('.highrise_filter_projects .highrise_filter_item li').removeClass('active');
        e.preventDefault();
    });
    ...
    $('.project-time-list-header .time-list-header-col span').on('click', function (e) {
        ...
        $('.highrise_timeline_list').isotope({
            sortBy: sortingValue
        });
    ...
    function timelineImageHeight() {
        ...
        setTimeout(function () {
            ...
            var timelineWrapper = $('.highrise_timeline'),
    ...
    $('.highrise_timeline .project-timeline-item').on('click', function (e) {
    ...
    $('.highrise_timeline .project-timeline-item .time-item-link').on('click', function (e) {
        $('.highrise_timeline .project-timeline-item').off('click');
    });
 ...
 */
    ...
    function stickyTimeline() {
        ...
        var stickyTop = $('.highrise_exhibition_timeline').offset().top,
    ...
    $('.project-detail-full-overlay .icon').on('click', function () {
        ...
        $('.highrise-header').toggleClass('open-detail');
    })
    ...
    /* PROJECT CATEGORIES */
    /**********************************/
    if ($('.highrise-categories-outer').length) {

        var categoriesOuter = $('.highrise-categories-outer');

        $(categoriesOuter).each(function () {
            var _this = $(this);
            if (!($(_this).prev('.highrise-categories-outer').length)) {
                $(_this).find('.categories_btn_up').addClass('hide')
            }
            if (!($(_this).next('.highrise-categories-outer').length)) {
                $(_this).find('.categories_btn_down').addClass('hide')
            }
        })
    }

    $('.highrise_categories_btn .categories_btn_up').on('click', function () {

        var prevCategory = $(this).closest('.highrise-categories-outer').prev('.highrise-categories-outer').offset().top;
    ...
    $('.highrise_categories_btn .categories_btn_down').on('click', function () {

        var nextCategory = $(this).closest('.highrise-categories-outer').next('.highrise-categories-outer').offset().top;
    ...
    function highriseGrid() {
        var block = '.highrise_services, .highrise_books, .highrise_media, .highrise_exhibition_grid, .highrise_grid';
    ...
    function highriseMasonry() {
        var block = '.highrise_masonry';
        if ($(block).length) {
    ...
    function highriseTimelineList() {
        var block = '.highrise_timeline_list';
        if ($(block).length) {
    ...
    $('.protect-page').on('submit', function (e) {
        ...
        if ($('#pwbox-1234').val() == "enter") {
            ...
            $('.highrise-protected-content p').html('<p>(Please, add the password)</p>');
        } else {
            $('.highrise-protected-content p').html('<p>(You entered the wrong password)</p>');
        }
        ...
        if (pageNum <= max) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ($container.hasClass('highrise-blog-grif-outer') || $container.hasClass('highrise_blog')) {
                        // console.log('second');
    ...
    pageCalculations(function () {
        wpc_add_img_bg('.s-img-switch');
        pageHeightCalculate();
        pageOnlyFullHeight('.highrise_filmstrip');
...
// Smooth scroll fix for redirected anchors
